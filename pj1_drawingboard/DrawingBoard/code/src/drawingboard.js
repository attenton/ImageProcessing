(function() {
	...
	function DBCtrl($scope) {
		...
		dbc.canvas.setHeight(780);
		dbc.canvas.setWidth(1080);
		dbc.bgColor = 'red';
		...
		dbc.drwaFree = function() {
			dbc.endDrawing();
			var fre, isDown, origX, origY;
			...
			dbc.canvas.observe('mouse:down', function(o){
				...
				origX = pointer.x;
				origY = pointer.y;
				fre = new fabric.Path('M 0 0 L 200 100 L 170 200 ');
				fre.set({ left: 120, top: 120,fill:'red' });
				dbc.canvas.add(fre);
			});
			...
			dbc.canvas.observe('mouse:move', function(o){
				...
				var pointer = dbc.canvas.getPointer(o.e);
				fre.set({ left: 120, top: 120,fill:'red' })
				fre.set({ width: Math.abs(origX - pointer.x),height: Math.abs(origY - pointer.y)});
				dbc.canvas.renderAll();
			});
